...
def fetch_table_schema_cached(db_config_str: str, schema_name: str, table_name: str) -> tuple:
    ...
    try:
        ...
        table_description = table_description_result[0] if table_description_result and table_description_result[
            0] else None
...
def generate_column_description(table_description: str, column_name: str, data_type: str, table_name: str,
    ...
    try:
        ...
        raise HTTPException(status_code=500,
                            detail=f"Error generating column description for {column_name}: {traceback.format_exc()}")
...
# def validate_and_fix_sql(query: str, schema_context: str) -> str:
...
def validate_and_fix_sql(query: str, schema_context: str) -> str:
    ...
    try:
        ...
        stages = [
            ...
            Database Schema:
            {schema_context}
            Task: For every column referenced in the query, check that it exists in the appropriate table or CTE in the schema. If any referenced column does not exist, replace it with the placeholder unknown_column (keep the query executable). Output only the modified SQL query.
            Input Query:
            {{QUERY}}
            """,
    ...
    try:
        ...
        **Process:**
        ...
        if decomposition_needed:
            ...
            fixed_sub_queries = [validate_and_fix_sql(q, schema_context) for q in sub_queries]
            final_query = validate_and_fix_sql(final_query, schema_context) if final_query else ""
